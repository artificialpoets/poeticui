    ...
    >
      <Headless.Select
        ...
        className={cx(
          ...
          // Background color
          "bg-transparent dark:bg-muted/30 dark:*:bg-popover",
          // Hide default focus styles
          "focus:outline-hidden",
          // Invalid state
          "data-invalid:border-destructive data-invalid:data-hover:border-destructive",
          // Disabled state
          "data-disabled:border-muted-foreground/30 data-disabled:opacity-100 data-disabled:bg-muted/20",
        )}
      />
      {!multiple && (
        <span className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-2">
          <svg
            className="size-5 stroke-muted-foreground group-has-data-disabled:stroke-muted-foreground/60 sm:size-4 forced-colors:stroke-[CanvasText]"
            viewBox="0 0 16 16"
            aria-hidden="true"
            fill="none"
          >
